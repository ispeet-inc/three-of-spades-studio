...
export const GameOverModal = () => {
  ...
  return (
    <Dialog open={true}>
      <DialogContent className={`${isMobile ? 'max-w-sm mx-4' : 'max-w-lg'} bg-gradient-to-br from-felt-green-light to-felt-green-dark border-2 border-gold/40 shadow-elevated backdrop-blur-sm`}>
        <DialogHeader className="text-center mb-4">
          <DialogTitle className={`${isMobile ? 'text-2xl' : 'text-3xl'} font-casino text-gold mb-2 flex items-center justify-center gap-2`}>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-casino-black/30 rounded-2xl p-4 border border-gold/30">
            <h3 className={`${isMobile ? 'text-lg' : 'text-xl'} font-semibold text-gold mb-3 text-center uppercase tracking-wide flex items-center justify-center gap-2`}>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className={`text-center p-3 rounded-xl border-2 transition-all duration-500
                ...
                <div className="flex items-center justify-center gap-1 mb-2">
                  {winningTeam === 0 && <Crown className={`${isMobile ? 'w-4 h-4' : 'w-5 h-5'} text-gold`} />}
                  <div className={`${isMobile ? 'text-sm' : 'text-base'} font-bold text-gold`}>Bidding Team</div>
                  {winningTeam === 0 && <Crown className={`${isMobile ? 'w-4 h-4' : 'w-5 h-5'} text-gold`} />}
                </div>
                <div className={`${isMobile ? 'text-3xl' : 'text-4xl'} font-casino font-bold text-gold mb-1`}>{scores[0]}</div>
                {winningTeam === 0 && (
              ...
              <div className={`text-center p-3 rounded-xl border-2 transition-all duration-500
                ...
                <div className="flex items-center justify-center gap-1 mb-2">
                  {winningTeam === 1 && <Crown className={`${isMobile ? 'w-4 h-4' : 'w-5 h-5'} text-gold`} />}
                  <div className={`${isMobile ? 'text-sm' : 'text-base'} font-bold ${winningTeam === 1 ? "text-gold" : "text-blue-300"}`}>Defending Team</div>
                  {winningTeam === 1 && <Crown className={`${isMobile ? 'w-4 h-4' : 'w-5 h-5'} text-gold`} />}
                </div>
                <div className={`${isMobile ? 'text-3xl' : 'text-4xl'} font-casino font-bold mb-1 ${winningTeam === 1 ? "text-gold" : "text-blue-300"}`}>
                  {scores[1]}
                </div>
                <div className="text-xs text-gold/70 font-medium tracking-wider">
          ...
          <div className="text-center pt-2">
            ...
            >
              <div className="flex items-center justify-center gap-2">
                <Sparkles className={`${isMobile ? 'w-4 h-4' : 'w-5 h-5'} group-hover:animate-spin`} />
                🎮 Play Again
                <Sparkles className={`${isMobile ? 'w-4 h-4' : 'w-5 h-5'} group-hover:animate-spin`} />
              </div>
            </Button>
